...
//                alert(data[0].ThirtyWott);
     ...
     $.ajax({
            ...
            success: function (data) {
            	 ...
            	 var res = data.split(",");
            	 var twentyWott= res[0];
            	 var thirtyWott=res[1];
            	 var fiftyWott=res[2];
...
//                var twentPerctage=Math.round(((twentyWott)/(twentyWott+thirtyWott+fiftyWott))*100);
//                var thirtyPerctage=Math.round(((thirtyWott)/(twentyWott+thirtyWott+fiftyWott))*100);
////                var fiftyPerctage=Math.round((fiftyWott*100)/(twentyWott+thirtyWott+fiftyWott+sixyWott));
//                var SixtyPerctage=Math.round(((sixyWott)/(twentyWott+thirtyWott+fiftyWott))*100);
//               
//               alert(twentyWott+" "+thirtyWott+" "+ fiftyWott +" "+sixyWott);
//               var thirtyvot=data[0].ThirtyWott;
               
        
               Morris.Donut({element:"do-morris-donut-chart",data:[{label:"6 Watt",value:Math.round(twentyWott)+"%"},
           {label:"30 Watt",value:Math.round(thirtyWott)+"%"},{label:"60 Watt",value:Math.round(fiftyWott)+"%"}],
...
var MorrisChart=function(){"use strict";
